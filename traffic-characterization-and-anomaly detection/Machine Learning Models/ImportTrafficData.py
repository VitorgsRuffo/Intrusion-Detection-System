...
class ImportTrafficData:
    ...
    @staticmethod
    def in_gru_regression_model_20_sec_format(path: str, anomalous_intervals: list):
        # Lendo os arquivos que contêm os atributos dos fluxos ips de um dia completo.
        # Obs: Eles estao separados segundo a segundo.
        bytes_count = np.loadtxt(os.path.join(path, 'bytes1.txt')).reshape(-1, 1)  # shape: (86400
        dst_ip_ent = np.loadtxt(os.path.join(path, 'EntropiaDstIP1.txt')).reshape(-1, 1)
        ...
        try:
            ...
            bytes_count_scaler = MinMaxScaler().fit(bytes_count)
            dst_ip_ent_scaler = MinMaxScaler().fit(dst_ip_ent)
            dst_port_ent_scaler = MinMaxScaler().fit(dst_port_ent)
            src_ip_ent_scaler = MinMaxScaler().fit(src_ip_ent)
            src_port_ent_scaler = MinMaxScaler().fit(src_port_ent)
            packets_count_scaler = MinMaxScaler().fit(packets_count)
            scalers = [bytes_count_scaler, dst_ip_ent_scaler, dst_port_ent_scaler,
                       src_ip_ent_scaler, src_port_ent_scaler, packets_count_scaler]
            _file = open('scalers.pkl', 'wb')
            dump(scalers, _file)
        ...
        data_y = np.copy(raw_data_x)[20:]
        data_x = np.zeros((data_y.shape[0], 20, 6))
        for i in range(data_y.shape[0]):
            data_x[i] = raw_data_x[i:i+20]
    ...
    def in_regression_model_20_sec_ugr16_format(path: str):
        # Lendo os arquivos que contêm os atributos dos fluxos ips de um dia completo.
        # Obs: Eles estao separados segundo a segundo.
        data_frame = pd.read_csv(path, dtype={"bytes": np.int64, "packets": np.int64, "label": np.int32})
