...
class AnomalyDetector:
    def __init__(self):
        ...
        self.thresholds = [-1, -1, -1, -1, -1, -1]

    def set_thresholds(self, x, y, labels):
        for i in range(0, 6):
            prediction = self.models[i].predict(x) #shape: (86399, 1)
            absolute_error = abs(prediction - y[:, i].reshape(-1, 1))
            best_f1_score = -1
            best_threshold = -1
            ...
            for threshold in interval:
                ...
                ans = np.zeros_like(absolute_error)

                # Os segundos nos quais o erro é maior que o threshold sao marcados como anomalos:
                ans[absolute_error > threshold] = 1
            ...
            #salvando o threshold encontrado para a variacao do atributo i.
            self.thresholds[i] = best_threshold

    def detect(self, x, y) -> list:
        # Vamos usar um modelo e threshold diferentes para prever cada um dos atributos.
        # Se pelo menos um dos atributos observados sairem do seus respectivos thresholds
        # em um certo segundo do dia, vamos indicar aquele segundo como anomalo.
        answer = np.zeros((86399, 1))
        for i in range(0, 6):
            prediction = self.models[i].predict(x)
            absolute_error = abs(prediction - y[:, i].reshape(-1, 1))
            threshold = self.thresholds[i]
            answer[absolute_error > threshold] = 1
        return answer
